var toTwos = function(buff) {
  ...
  if(neg) {
    ...
    if(buff[buff.length - 1] == 255)
    	console.log('Fuck twos complement.', buff);
    buff[buff.length - 1]++;
  }
...
module.exports = function(data) {
	...
	for (i = 0; i < data.length; i++) {
		j = data[i];
		if(j.byte !== undefined || j === true || j === false)
			len += 1;
		else if(j.uint32 !== undefined)
			len += 4;
		else if(j.raw)
			len += j.raw.length;
			...
			len += 4 + toTwos(j.mpint).length;
		else
			console.log('What size is', j);
	}
	...
	for (i = 0; i < data.length; i++) {
		...
		if(j.byte !== undefined)
			payload.writeUInt8(j.byte, (idx += 1) - 1);
		else if(j.uint32 !== undefined)
			payload.writeUInt32BE(j.uint32, (idx += 4) - 4);
		else if(j.raw)
			j.raw.copy(payload, (idx += j.raw.length) - j.raw.length);
		else if(j.length !== undefined) {
